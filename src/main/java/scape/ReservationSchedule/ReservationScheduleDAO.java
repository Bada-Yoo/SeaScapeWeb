package scape.ReservationSchedule;

import scape.activate.DBUtil;
...
public class ReservationScheduleDAO {
    ...
    public List<ReservationScheduleDTO> selectReservationsByUser(String userId) {
        ...
        try (Connection conn = DBUtil.getConnection();
            ...
            while (rs.next()) {
                ReservationScheduleDTO dto = makeReservationSet(rs);
    ...
    // RoomDTO 생성 메서드
    private RoomDTO makeRoomSet(ResultSet rs) throws SQLException {
    ...
    // ReservationScheduleDTO 생성 메서드
    private ReservationScheduleDTO makeReservationSet(ResultSet rs) throws SQLException {
        return ReservationScheduleDTO.builder()
            .SCHEDULE_ID(rs.getInt("schedule_id"))
            .ROOM_ID(rs.getString("room_id"))
